...
import type { ErpRow } from "@/lib/bom/types";
...
export function ErpPreviewTable({ rows, open, onToggle }: Props) {
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr style={{ background: "#EDF2F7" }}>
                ...
                <th className="px-2 py-2 text-left font-semibold uppercase">SHORT NAME</th>
            ...
            <tbody>
              {visible.map((r, i) => (
                <tr key={i} className={i % 2 ? "bg-gray-50" : "bg-white"}>
                  <td className="px-2 py-1.5">{String(r.slNo)}</td>
                  <td className="px-2 py-1.5">{String(r.bomLevel)}</td>
                  <td className="px-2 py-1.5">{r.itemCode}</td>
                  <td className="px-2 py-1.5">{r.shortName}</td>
